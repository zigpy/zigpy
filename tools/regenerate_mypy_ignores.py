...
def strip_overrides(content: str) -> str:
    """Remove the autogenerated per-module overrides block from pyproject content.

    This leaves the base `[tool.mypy]` config (enable_error_code, strict, etc.)
    intact so mypy reports every error the project config enables.
    """
    pattern = re.compile(
...
def run_mypy(content: str) -> list[str]:
    """Run mypy using the project config but without the per-module overrides."""
    with tempfile.NamedTemporaryFile(
        mode="w", suffix=".toml", delete=False
    ) as config_file:
        config_file.write(strip_overrides(content))

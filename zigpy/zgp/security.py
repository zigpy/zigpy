...
def calculate_mic(key: bytes, nonce: bytes, authData: bytes, plaintextData: bytes) -> bytes:
...
def zgp_encrypt(key: bytes, nonce: bytes, authHeader: bytes, plaintextData: bytes) -> tuple[bytes, bytes]:
  assert len(nonce) == 13
...
def zgp_decrypt(key: bytes, nonce: bytes, authHeader: bytes, ciphertext: bytes, enc_mic: bytes) -> tuple[bytes, bool, bytes]:
  assert len(nonce) == 13

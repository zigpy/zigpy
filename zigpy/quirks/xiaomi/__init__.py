...
class MotionSensor(CustomDevice):
    class OccupancyCluster(CustomCluster, OccupancySensing):
        ...
        def _turn_off(self):
            self._timer_handle = None
            self._update_attribute(self, 0, 0)

    signature = {
        # <SimpleDescriptor endpoint=1 profile=260 device_type=260 device_version=1 input_clusters=[0, 3, 65535, 25] output_clusters=[0, 4, 3, 6, 8, 5, 25]>
        1: {
            'profile_id': 0x0104,
            'device_type': 0x0104,
    ...
    replacement = {
        'endpoints': {
            1: {
                'device_type': 0x0107,
                'input_clusters': [0x0000, 0x0003, OccupancyCluster],
            },
        },
    }

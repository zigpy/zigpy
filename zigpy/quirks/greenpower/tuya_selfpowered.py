...
class TuyaSelfPoweredSwitch(GreenPowerDevice):
    ...
    def match(cls, device: GreenPowerDevice) -> bool:
        if device.ext_data.device_id is not GPDeviceType.SWITCH_ON_OFF:
            return False
        if device.ext_data.security_level is not GPSecurityLevel.Encrypted:
            return False
        if device.ext_data.security_key_type is not GPSecurityKeyType.IndividualKey:
            return False

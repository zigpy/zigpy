...
This software is aimed at application developers who wish to incorporate or integrate Zigbee functionality into their applications. The ultimate goal is for zigpy to be a free software library that interfaces with Zigbee controllers called "coordinators" in Zigbee terminology, allowing developers who integrates zigpy to create applications to control devices on a Zigbee network, without requiring very in-depth knowledge of the Zigbee protocols or deep understanding on the Zigbee specifcations. The zigpy projects is as such not about creating Zigbee devices (routers and end devices, like power-relays and sensors). The project consists of the main zigpy library, wrappers for Zigbee radios from different manufacturers, and supporting projects, all written in Python.

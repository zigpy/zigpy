...
@patch(
    "zigpy.device.OTA_RETRY_DECORATOR",
    zigpy.util.retryable_request(tries=4, delay=0.01),
)
async def test_update_firmware_slow_rebooting_device(monkeypatch, dev):
    """Slow-rebooting device: read_attributes fails a few times before the
    device comes back, but image_notify and reinterview() must still run.
    """
    ep = dev.add_endpoint(1)
    cluster = zigpy.zcl.Cluster.from_id(ep, Ota.cluster_id, is_server=False)
    ep.add_output_cluster(Ota.cluster_id, cluster)
    ...
    async def flaky_read_attributes(*args, **kwargs):
        nonlocal read_attempts
        read_attempts += 1
        if read_attempts < 3:
            raise zigpy.exceptions.DeliveryError("device still rebooting")
        return ([], [])

    cluster.read_attributes = flaky_read_attributes

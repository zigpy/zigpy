...
async def test_ota_manager_sonoff_uses_48_byte_blocks():
    """Test Sonoff devices can request and receive 48-byte OTA image blocks."""

    sonoff_fw_image = zigpy.ota.OtaImageWithMetadata(
        metadata=zigpy.ota.providers.BaseOtaImageMetadata(
            file_version=0x12345678,
    ...
    with mock_attribute_reads(
        cluster,
        {"current_file_version": sonoff_fw_image.firmware.header.file_version - 10},
    ):
        await dev.initialize()
    ...
    async def send_packet(packet: t.ZigbeePacket):
        ...
        hdr, cmd = cluster.deserialize(packet.data.serialize())
        assert sonoff_fw_image.firmware is not None
        ...
        if isinstance(cmd, Ota.ImageNotifyCommand):
            dev.application.packet_received(
                make_packet(
                    ...
                    "query_next_image",
                    field_control=Ota.QueryNextImageCommand.FieldControl.HardwareVersion,
                    manufacturer_code=sonoff_fw_image.firmware.header.manufacturer_id,
                    image_type=sonoff_fw_image.firmware.header.image_type,
                    current_file_version=(
                        sonoff_fw_image.firmware.header.file_version - 10
                    ),
                    hardware_version=1,
                )
            )
            ...
            assert cmd.status == foundation.Status.SUCCESS
            assert (
                cmd.manufacturer_code == sonoff_fw_image.firmware.header.manufacturer_id
            )
            ...
            dev.application.packet_received(
                make_packet(
                    ...
                    manufacturer_code=sonoff_fw_image.firmware.header.manufacturer_id,
                    image_type=sonoff_fw_image.firmware.header.image_type,
                    file_version=sonoff_fw_image.firmware.header.file_version,
                    file_offset=0,
            ...
            if cmd.file_offset == 0:
                assert len(cmd.image_data) == 48
                assert cmd.image_data == sonoff_fw_image.firmware.serialize()[0:48]
                dev.application.packet_received(
                    make_packet(
                        dev,
                        cluster,
            ...
            elif cmd.file_offset == 48:
                assert cmd.image_data == sonoff_fw_image.firmware.serialize()[48:70]
                dev.application.packet_received(
                    make_packet(
                        dev,
                        cluster,
    ...
    dev.application.send_packet = AsyncMock(side_effect=send_packet)
    progress_callback = MagicMock()
    result = await update_firmware(dev, sonoff_fw_image, progress_callback)

    assert progress_callback.call_count == 2
    assert progress_callback.call_args_list[0] == call(48, 70, pytest.approx(68.5714))
